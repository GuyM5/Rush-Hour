...
def main ():
    ...
    scheduler = torch.optim.lr_scheduler.StepLR(optim,100000 * 10, gamma=0.50) #בנתיים יותר טוב עם 1000 * 10 מאשר 100000* 10 
    
    for epoch in range(start_epoch, epochs):
        
        state_1 = env.random_init_state(shuffle_time=100,cars_num=min(epoch//12000 + 1, 12))
        while True: #not env.is_end_of_game(state_1) :
            ...
            if after_state_1.step > 200:
                buffer.push(state_1, action_1, reward_1, after_state_1, False) # fix reward
                break
